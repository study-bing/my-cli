 ...
 * @LastEditTime: 2022-02-25 15:12:46
 * @LastEditors: linbin
 * @Description: 指令
...
const { ejsCompile, writeFile, mkdirSync, validateName, firstToUpper } = require('../utils/file')
...
const createProject = async project => {
	...
	const command = process.platform === 'win32' ? `${installType}.cmd` : installType
...
const addComponent = async (name, dest) => {
	let vue = await versionChoose()
	src = '../template/component.vue.ejs'
	if (vue === 'vue3') {
