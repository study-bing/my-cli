 ...
 * @LastEditTime: 2022-01-27 10:36:54
 * @LastEditors: linbin
 * @Description:
 * @FilePath: /my-cli/lib/core/create.js
 */
const { program } = require('commander')
const { createProject, addComponent, addPage, addStore, updateChk } = require('./actions')
const options = program.opts()
const createCommands = () => {
	// 配置参数 program.
	program.command('create <project> [otherArgs...]').description('clone a repository into a newly created directory').action(createProject)
	// 创建组件.
	program
		.command('addcpn <name>')
